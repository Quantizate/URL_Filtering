...
int main(int argc, char **argv)
{
  ...
  // xorbinaryfusefilter_naive::XorBinaryFuseFilter<uint64_t, uint8_t> xbf_8_naive(FILTER_SIZE);

  // // Construction
  // xbf_8_naive.AddAll(test_hashes, 0, TEST_SIZE);

  // // Let us check the size of the filter in bytes:
  // size_t filter_volume = xbf_8_naive.SizeInBytes();
  ...
  // xorbinaryfusefilter_naive::XorBinaryFuseFilter<uint64_t, uint16_t> xbf_16_naive(FILTER_SIZE);
  ...
  // {
  //   bool in_set = xbf_16_naive.Contain(ref);
  //   if (in_set)
  //   {
  //     fpp++;
  //   }
  ...
  xorbinaryfusefilter_lowmem::XorBinaryFuseFilter<uint64_t, uint16_t> xbf_16(FILTER_SIZE);
  ...
  for (auto &ref : bogus_hashes)
  {
    bool in_set = xbf_16.Contain(ref);
    if (in_set)
    {
      fpp++;
    }
  ...
  writeOutput("Xor Binary Fuse - 16", truePositive, trueNegative, falsePositive, falseNegative, outputFile, fpp);
  ...
  xorbinaryfusefilter_lowmem4wise::XorBinaryFuseFilter<uint64_t, uint16_t> xbf_16_4(FILTER_SIZE);
  ...
  for (auto &ref : bogus_hashes)
  {
    bool in_set = xbf_16_4.Contain(ref);
    if (in_set)
    {
      fpp++;
    }
  ...
  writeOutput("Xor Binary Fuse - 16 4-wise", truePositive, trueNegative, falsePositive, falseNegative, outputFile, fpp);
