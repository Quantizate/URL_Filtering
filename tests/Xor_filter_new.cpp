...
int main(int argc, char **argv)
{
  ...
  XorFilter<uint64_t, uint16_t> filter_16(filter_size), filter_16_test(filter_size);

  // Construction

  is_ok = filter_16.AddAll(hashes.data(), 0, size);
  if (is_ok)
  {
    // printf("Failed to add all elements to the filter\n");
    return EXIT_FAILURE;
  }
  ...
  for (int i = 0; i < bogus_size; i++)
  {
    bool in_set = 1 - filter_16.Contain(bogus_hashes[i]);
    if (in_set)
    {
      fp_bogus++;
    }
  ...
  pretty_print(test_hashes.size(), bytes,
               bench([&test_hashes, &filter_16_test, &size]()
                     { filter_16_test.AddAll(test_hashes.data(), 0, size); }),
               stat2);
  ...
  for (int i = 0; i < data_size; i++)
  {
    dataValidity[i].second = 1 - filter_16.Contain(bogus_hashes[i]);
  }
