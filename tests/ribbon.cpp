...
int main(int argc, char **argv)
{
   ...
   */
  HomogRibbonFilter<uint64_t, 5> hr5(TEST_SIZE), test_filter(FILTER_SIZE);

  // Construction
  hr5.AddAll(test_hashes, 0, TEST_SIZE);

  // Let us check the size of the filter in bytes:
  size_t filter_volume = hr5.SizeInBytes();
  ...
  for (int i = 0; i < BOGUS_SIZE; i++)
  {
    ...
    bool in_set = hr5.Contain(bogus_hashes[i]);
    if (in_set)
    {
      fpp++;
    }
  ...
  // HomogRibbonFilter<uint64_t, 15> hr15(TEST_SIZE);
  ...
  // BalancedRibbonFilter<uint64_t, 5, 0> br5(TEST_SIZE);
  ...
  // BalancedRibbonFilter<uint64_t, 15, 0> br15(TEST_SIZE);
  ...
  StandardRibbonFilter<uint64_t, 7, 25> sr7_25(TEST_SIZE);
  ...
  for (auto &ref : bogus_hashes)
  {
    bool in_set = sr7_25.Contain(ref);
    if (in_set)
    {
      fpp++;
    }
  ...
  BalancedRibbonFilter<uint64_t, 7, 10> sr7_10(TEST_SIZE);
  ...
  for (auto &ref : bogus_hashes)
  {
    bool in_set = sr7_10.Contain(ref);
    if (in_set)
    {
      fpp++;
    }
  ...
  BalancedRibbonFilter<uint64_t, 15, 0> sr15(TEST_SIZE);
