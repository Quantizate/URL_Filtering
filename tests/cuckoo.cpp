...
{
  FILE *outputFile = fopen("values.csv", "a");
  ...
  CuckooFilterStable<uint64_t, 8> filter(TEST_SIZE), filter4(TEST_SIZE);
  ...
  //                  basic_count +=
  //                      1 - filter.Contain(simple_hash(ref));
  //                } }));

  printf("Benchmarking construction speed\n");
  volatile int i = 0;
  pretty_print(test_hashes.size(), bytes, "binary_fuse16_populate",
               bench([&test_hashes, &filter4, &i]()
                     {
                     for(i = 0; i < test_hashes.size(); i++){
                        filter4.Add(test_hashes[i]);
                        i++; } }));
  ...
  writeOutput("Cuckoo-8", truePositive, trueNegative, falsePositive, falseNegative, outputFile, fpp);
  ...
  CuckooFilterStable<uint64_t, 16> filter3(TEST_SIZE);
  ...
  writeOutput("Cuckoo-16", truePositive, trueNegative, falsePositive, falseNegative, outputFile, fpp);

  printf("-------------- Cuckoo Fuse - 8 Filter --------------\n");
  CuckooFuseFilter<uint64_t, uint8_t> fuse_8(TEST_SIZE);

  for (int i = 0; i < TEST_SIZE; i++)
  {
    fuse_8.Add(test_hashes[i]);
  }

  // Let us check the size of the filter in bytes:
  filter_volume = fuse_8.SizeInBytes();
  printf("\nfilter memory usage : %zu bytes (%.1f %% of input)\n", filter_volume,
         100.0 * filter_volume / bytes);
  printf("\nfilter memory usage : %1.f bits/entry\n",
         8.0 * filter_volume / test_hashes.size());
  ...
  for (auto &ref : bogus_hashes)
  {
    bool in_set = 1 - fuse_8.Contain(ref);
    if (in_set)
    {
      fpp++;
    }
  ...
  for (int i = 0; i < DATA_SIZE; i++)
  {
    dataValidity[i].second = 1 - fuse_8.Contain(hashes[i]);
  }
  ...
  writeOutput("Cuckoo Fuse-8", truePositive, trueNegative, falsePositive, falseNegative, outputFile, fpp);

  printf("-------------- Cuckoo Fuse - 16 Filter --------------\n");
  CuckooFuseFilter<uint64_t, uint16_t> fuse_16(TEST_SIZE);
  ...
  for (auto &ref : bogus_hashes)
  {
    bool in_set = 1 - fuse_16.Contain(ref);
    if (in_set)
    {
      fpp++;
    }
  ...
  writeOutput("Cuckoo Fuse-16", truePositive, trueNegative, falsePositive, falseNegative, outputFile, fpp);
