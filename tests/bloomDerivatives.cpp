...
int main(int argc, char **argv)
{
  ...
  BloomFilter<uint64_t, 8, true> bBloom_8(FILTER_SIZE), test_filter(FILTER_SIZE);

  // Construction
  bBloom_8.AddAll(test_hashes, 0, TEST_SIZE);

  // Let us check the size of the filter in bytes:
  size_t filter_volume = bBloom_8.SizeInBytes();
  ...
  BloomFilter<uint64_t, 16, true> bBloom_16(FILTER_SIZE);
  ...
  {
    bool in_set = 1 - bBloom_16.Contain(ref);
    if (in_set)
    {
      fpp++;
    }
  ...
  SimpleBlockFilter<8, 8> blockedBloom_8(FILTER_SIZE);

  for (int i = 0; i < TEST_SIZE; i++)
    blockedBloom_8.Add(test_hashes[i]);

  // Let us check the size of the filter in bytes:
  filter_volume = blockedBloom_8.SizeInBytes();
  printf("\nfilter memory usage : %zu bytes (%.1f %% of input)\n", filter_volume,
         100.0 * filter_volume / bytes);
  printf("\nfilter memory usage : %1.f bits/entry\n",
         8.0 * filter_volume / test_hashes.size());
  ...
  for (auto &ref : bogus_hashes)
  {
    bool in_set = blockedBloom_8.Find(ref);
    if (in_set)
    {
      fpp++;
    }
  ...
  for (int i = 0; i < DATA_SIZE; i++)
  {
    dataValidity[i].second = blockedBloom_8.Find(hashes[i]);
  }
  ...
  printf("-------------- Counting Bloom 10 Filter --------------\n");
  BloomFilter<uint64_t, 10, true> cntBloom10(FILTER_SIZE);

  // Construction
  cntBloom10.AddAll(test_hashes, 0, TEST_SIZE);

  // Let us check the size of the filter in bytes:
  filter_volume = cntBloom10.SizeInBytes();
  printf("\nfilter memory usage : %zu bytes (%.1f %% of input)\n", filter_volume,
         100.0 * filter_volume / bytes);
  printf("\nfilter memory usage : %1.f bits/entry\n",
         8.0 * filter_volume / test_hashes.size());
  ...
  for (auto &ref : bogus_hashes)
  {
    bool in_set = 1 - cntBloom10.Contain(ref);
    if (in_set)
    {
      fpp++;
    }
  ...
  for (int i = 0; i < DATA_SIZE; i++)
  {
    dataValidity[i].second = 1 - cntBloom10.Contain(hashes[i]);
  }
  ...
  printf("-------------- Succinct Counting Bloom 10 Filter --------------\n");
  SuccinctCountingBloomFilter<uint64_t, 10, true> scountBloom(FILTER_SIZE);
  ...
  for (auto &ref : bogus_hashes)
  {
    bool in_set = 1 - scountBloom.Contain(ref);
    if (in_set)
    {
      fpp++;
    }
  ...
  SuccinctCountingBlockedBloomFilter<uint64_t, 10, SimpleMixSplit> sCntBlBloom(FILTER_SIZE);

  // Construction
  for (int i = 0; i < TEST_SIZE; i++)
    sCntBlBloom.Add(test_hashes[i]);
  ...
  for (auto &ref : bogus_hashes)
  {
    bool in_set = sCntBlBloom.Contain(ref);
    if (in_set)
    {
      fpp++;
    }
  ...
  for (int i = 0; i < DATA_SIZE; i++)
  {
    dataValidity[i].second = sCntBlBloom.Contain(hashes[i]);
  }
  ...
  // Construction
  for (int i = 0; i < TEST_SIZE; i++)
    scbrBloom.Add(test_hashes[i]);

  // Let us check the size of the filter in bytes:
  filter_volume = scbrBloom.SizeInBytes();
  printf("\nfilter memory usage : %zu bytes (%.1f %% of input)\n", filter_volume,
         100.0 * filter_volume / bytes);
  printf("\nfilter memory usage : %1.f bits/entry\n",
         8.0 * filter_volume / test_hashes.size());
  ...
  for (auto &ref : bogus_hashes)
  {
    bool in_set = scbrBloom.Contain(ref);
    if (in_set)
    {
      fpp++;
    }
  ...
  for (int i = 0; i < DATA_SIZE; i++)
  {
    dataValidity[i].second = scbrBloom.Contain(hashes[i]);
  }
