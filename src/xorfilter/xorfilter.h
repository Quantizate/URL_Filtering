...
namespace xorfilter
{
    ...
    __attribute__((always_inline)) inline uint64_t rotl64(uint64_t n, unsigned int c)
    {
    ...
    __attribute__((always_inline)) inline size_t getHashFromHash(uint64_t hash, int index, int blockLength)
    {
    ...
    {
        //   std::stringstream ss;  // For linux
        std::basic_stringstream<char> ss; // For mac
        ss << "XorFilter Status:\n"
           << "\t\tKeys stored: " << Size() << "\n";
        return ss.str();
    }
    ...
    namespace naive
    {
        ...
        {
            std::stringstream ss; // For linux
            // std::basic_stringstream ss; // For mac
            ss << "XorFilter Status:\n"
               << "\t\tKeys stored: " << Size() << "\n";
            return ss.str();
        }
